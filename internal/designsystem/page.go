...
// ── The page model ───────────────────────────────────────────────────
//
// Two path prefixes decide where every relative link on a page points,
// and they are the only difference between docs/design-system/index.html
// and docs/design-system/ink/en/index.html:
//
//	Root  how to get to the tree root  ("" or "../../")
//	Self  how to get to this page's own theme/locale directory
//	      ("ink/en/" from the root, "" from inside it)
//
// Every asset is Root-relative and every shell demo is Self-relative, so
// the root copy is generated by passing different prefixes rather than
// by copying bytes and patching them.
...
// renderIndex builds one theme × locale index page. root and self are
// the two path prefixes described above.
func renderIndex(theme, locale, root, self string) ([]byte, error) {
...
func shellViews(self string) []shellView {
...
func renderShell(theme, locale, shell, root string) ([]byte, error) {
...
//   - Every href and src goes through .Root or .Self. The tree is served
//     from a subdirectory, so an absolute path would resolve at the site
//     root; a gate holds that too.
...
<style>` + dsCSS + `</style>
<script defer src="{{.Root}}rastrillo.js"></script>
<script defer src="{{.Root}}select.js"></script>
<script defer src="{{.Root}}datetime.js"></script>
</head>
<body>
