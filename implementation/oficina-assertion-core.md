...
Status: pure core contract approved by Claude Fable 5.1 at d23b871 on 2026-09-08; implementation under review. User approved prioritising common app entry → Home → team choice → team subdomain. This is the first shared dependency, not a claim that the whole journey exists.
...
- Canonical origins have ASCII lowercase DNS A-labels, no trailing dot, no IP literals (including browser-recognized shortened, decimal and hexadecimal IPv4 forms), no slash. Labels have 1..63 bytes, alphanumeric ends and only alphanumeric/hyphen interiors; total hostname at most 253 bytes. Optional explicit port is 1..65535, without leading zeros; explicit 443 is refused. Compare received issuer/audience byte-for-byte without normalization.
...
- Add assertion to the existing Makefile race target; the existing ci.d step invokes that target and remains unchanged because the CI contract requires a bare shebang plus exec make race. The unconditional independent fixture verifies a fixed external P-256 signature; reverse-direction interoperability uses Node when available and never compares randomized signatures.

Implementation evidence: raw-byte parser fixtures and a Node WebCrypto-generated golden signature pass. Node also verifies Go output. Separate Go overlays disabling audience and expiry checks each make their focused tests fail; the source stayed unchanged during mutation testing. Full CI and whole-code Fable review remain required.
