...
- **Modern CSS is the floor, not a hazard.** `tokens.css` plus a shipped
  theme already require Chrome 123, Safari 17.5, Firefox 121 — all 2024 —
  because every theme colour is a `light-dark()`. Reach for the modern
  feature: anything that shipped at or below that floor is free, `oklch()`
  and `color-mix()` included (two themes already use `color-mix()`), with
  no hex twin and no `@supports`. Pairing every colour with a hex fallback
  protects nothing anyway — an engine too old for `oklch()` dropped the
  whole `light-dark()` palette several declarations earlier. And
  self-contained means the CSS fetches nothing: no imports, no remote
  assets, no webfont, held by `ui_test.go`. It has never meant old
  engines. Go above the floor with `@supports`, or move the floor and say
  so — never lower it by accident.
  docs/site/templates.md
