...
Content addressing is what keeps this simple. Write-once means no state
machine and no lease, and a re-upload of the same bytes is idempotent by
construction.
...
The platform's object-storage primitive: the app's own bucket, with
credentials delivered as `CARLOS_STORE_*` environment variables.
**This is where blobs belong.**

`S3.Get`, `S3.Put` and `S3.Delete` are the store operations.
`S3.PresignGet` and `S3.PresignPut` mint presigned URLs locally, because
the platform deliberately ships no signing service — so a browser can
upload and download without the app proxying the bytes.

The SigV4 signer is hand-rolled, with no AWS SDK dependency, and is
pinned against the official AWS test vectors.
...
`InlineMax` is 4 KiB, and it is the working rule: stated everywhere,
enforced nowhere. Anything larger belongs in the object store, where
bytes do not ride every database page the row touches, do not bloat the
WAL stream the platform replicates, and can be served by presigned URL.
...
Two honest costs, both inherent rather than incidental: the address
becomes the **ciphertext's** hash, and dedup therefore happens per key
rather than globally. Two users storing identical bytes store them
twice.
