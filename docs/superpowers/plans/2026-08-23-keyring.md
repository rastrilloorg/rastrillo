...
**Tech Stack:** Go 1.25 (stdlib only: `crypto/ecdh`, `crypto/x509`, `embed`), `amadan.net/rastrillo/rastrillo/crypto`, WebCrypto ES module (Node ≥ 20 for the twin's tests, optional), `go test` + `node --test`.
...
**Interfaces:**
- Consumes: `crypto.Derive(key []byte, context string) []byte`, `crypto.NewKey() ([]byte, error)` (from `amadan.net/rastrillo/rastrillo/crypto`).
